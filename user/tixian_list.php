...
{
   $query = "Select * From `#@__amounts` where uid='".$cfg_ml->M_ID."' and ptype=1 order by ctime desc";
   $row= array('key'=>'','pstyle'=>-1,'state'=>-1);
   $dlist = new DataListCP();
    $dlist->pageSize = 5;
    $dlist->SetParameter('keyword',$keyword);
    $dlist->SetTemplate(DEDEMEMBER.'/templets/tx_list.htm');
    $dlist->SetSource($query);
    $dlist->Display();
}
if($dopost == 'search')
{
    $query="Select * From `#@__amounts` where uid='".$cfg_ml->M_ID."' and ptype=1  ";
    if($keyword !='')
